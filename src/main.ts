...
for (const line of [
  ...
  '「プロファイルを使う」の「読み込む」… 校正せずに、保存したプロファイルを使います。profile欄の上で右クリックして「読み込み」でファイルを選び、「適用する」を押すと、このカメラに使えるかを表示します',
  'i … profile欄に「読み込み」したプロファイルを適用します（「適用する」と同じ）。校正を終えた画面でも使えます',
...
for (const line of [
  ...
  '同時に、ステージ左の profile リストに校正データが1行ずつ入り、同じ内容がステージ右にQRコードで表示されます。形式はROSの camera_info YAML で、ROSやOpenCV系のツールもそのまま読めます。別の端末でQRコードを読み取るか、profile欄の項目上で右クリックして「書き出し」を選び、ファイルとして保存してください（profile.txt という名前で保存されます。ROSのツールで使うときは拡張子を .yaml に変えてください）。これが消費側アプリへの受け渡し経路です。',
